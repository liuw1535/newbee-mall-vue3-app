      ...
      <header class="category-header wrap">
        ...
        <div class="header-search">
          ...
          <input
            ...
            placeholder="请输入商品名称"
          />
          <i class="nbicon nb-close" v-if="state.keyword" @click="clearSearch" style="cursor:pointer;margin-left:5px;"></i>
        </div>
        <span class="search-btn" @click="getSearch">
          <span v-if="!state.loading">搜索</span>
      ...
      <van-pull-refresh v-model="state.refreshing" @refresh="onRefresh" class="product-list-refresh">
        ...
        >
          <template v-if="state.productList.length">
            <div class="product-item" v-for="(item, index) in state.productList" :key="index" @click="productDetail(item)">
              ...
              <div class="product-info">
                <p class="name" v-html="highlightKeyword(item.goodsName)"></p>
          ...
          <template v-else>
            ...
            <p v-if="!state.loading" class="empty-text">未找到相关商品</p>
          </template>
        </van-list>
      </van-pull-refresh>
    </div>
...
const highlightKeyword = (text) => {
  ...
  // 转义正则特殊字符
  const keyword = state.keyword.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
  const reg = new RegExp(`(${keyword})`, 'gi');
  return text.replace(reg, '<span style="color:#1baeae;font-weight:bold;">$1</span>');
}
</script>
