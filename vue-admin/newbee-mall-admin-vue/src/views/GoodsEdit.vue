...
import { getGoodsDetail, updateGoods, getCategoryList, uploadEditorImage } from '@/utils/api'
...
const getDetail = async () => {
  ...
  try {
    ...
    if (res.code === 200) {
      ...
      // 设置分类级联选择器的值
      if (formData.goodsCategoryId) {
        selectedCategory.value = findCategoryPath(formData.goodsCategoryId)
      }
    }
  } catch (error) {
...
const findCategoryPath = (categoryId) => {
  const path = []
  const findPath = (options, targetId, currentPath) => {
    for (const option of options) {
      if (option.categoryId === targetId) {
        return [...currentPath, option.categoryId]
      }
      if (option.children && option.children.length > 0) {
        const result = findPath(option.children, targetId, [...currentPath, option.categoryId])
